...
StrTnsr = 0.5*(fe.grad(v)+fe.grad(v).T)
mut     = Cu*(k**2)/e
P       = 2.0*mut*fe.inner(StrTnsr,StrTnsr)
...
RANSeqn =   rho*fe.inner(w,fe.grad(v)*v)*dx           \
           ...
           + mu*fe.inner(fe.grad(w),fe.grad(v))*dx    \
           - mut*fe.inner(fe.grad(w),fe.grad(v))*dx   \
           - mut*fe.inner(fe.grad(w),fe.grad(v).T)*dx \
           - fe.dot(w,fe.grad(k))*(2/3)*dx

Contieqn =  q*fe.div(v)*dx

keqn =   y*fe.dot(v,fe.grad(k))*dx                            \
