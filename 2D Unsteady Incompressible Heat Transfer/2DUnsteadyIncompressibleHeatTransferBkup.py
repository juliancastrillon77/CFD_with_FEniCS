...
dt    = 0.01 # Timestep             [s]
t_end = 7.7   # Length of simulation [s]
...
Egy0 = s*rho*cp*fe.dot(u0,fe.grad(T0))*dx    \
     + fe.dot(fe.grad(s), k*fe.grad(T0))*dx 
...
FileVel  = fe.File('ResultsT/Velocity.pvd')
FilePres = fe.File('ResultsT/Pressure.pvd')
FileTemp = fe.File('ResultsT/Temperature.pvd')
