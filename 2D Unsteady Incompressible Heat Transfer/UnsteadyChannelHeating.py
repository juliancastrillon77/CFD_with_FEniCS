...
Temp2 = fe.Constant(7)
alpha = fe.Constant(0.1) # Thermal diffusivity
...
mu  = fe.Constant(0.000825)    # Dynamic viscosity               [kg/ms]
rho = fe.Constant(1.225)       # Density                         [kg/m3]
k   = fe.Constant(0.008551)    # Thermal conductivity            [W/mK]
...
POut      = fe.Constant(0)
InletFlow = fe.Expression(['3.7*((4*x[1]*(0.1-x[1]))/(0.1*0.1))','0'], degree=2)
...
FileVel  = fe.File('Results2/Velocity.pvd')
FilePres = fe.File('Results2/Pressure.pvd')
FileTemp = fe.File('Results2/Temperature.pvd')
